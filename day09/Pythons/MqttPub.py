...
PUB_ID = 'IOT84' # 본인 아이피 마지막 주소
BROKER = '210.119.12.84' # 본인 아이피
PORT = 1883
TOPIC = 'smarthome/84/topic'    # publish/subscribe에서 사용할 토픽
...
try:
    ...
    while True:
        ...
        huimd = random.uniform(40.0, 65.9)   # [Fake] 습도, 실제로는 센서에서 값을 받아옴  
        ...
        raw_data['SENSING_DT'] = currtime.strftime('%Y-%m-%d %H:%M:%S') # C# 'yyyy-MM--dd HH:mm:ss
        raw_data['TEMP'] = f'{temp:0.1f}'   # 소수점 1자리만 전달
        raw_data['HUMID'] = f'{huimd:0.1f}'
